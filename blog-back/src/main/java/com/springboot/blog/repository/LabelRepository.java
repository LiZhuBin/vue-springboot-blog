...
public interface LabelRepository extends JpaRepository<Label,Integer> {
    ...
    @Query(nativeQuery = true,value = "select label.label_name,label.id from label where label.id = any(select article_to_label.label_id from article_to_label where article_to_label.article_id = any(select article.account_id from article where article.account_id=:#{#accountId}))")
